...
Admittance::Admittance(ros::NodeHandle &n,
  ...
  sub_ur5_dke_pos_         = nh_.subscribe("/ur5_dke_pos", 5, &Admittance::ur5_dke_pos_callback, this, ros::TransportHints().reliable().tcpNoDelay());
...
void Admittance::compute_admittance() {
  ...
  std::cout<<error.topRows(3)<<std::endl;
...
void Admittance::ur5_dke_pos_callback(const ur5_control::ur5_dke_pos::ConstPtr& msg)
{
  desired_pose_position_ << msg->x, msg->y, msg->z;
...
void Admittance::state_wrench_callback(
  ...
  if (ft_arm_ready_) {
    wrench_ft_frame <<  msg->wrench.force.x,-msg->wrench.force.z,msg->wrench.force.y,0,0,0;
